...
snakeHeadImg.src = 'headdd.png';
const snakeBodyImg = new Image();
snakeBodyImg.src = 'SnakeBody.png';
const foodImg = new Image();
foodImg.src = 'food3.png';
...
function update() {
    ...
    if (head.x === food.x && head.y === food.y) {
        score++;
        currentScoreElem.textContent = score;
        updateInterval = Math.max(10000, updateInterval - 20);
        food = {
            ...generateFoodPosition(),
            dx: (Math.random() < 0.5 ? 1 : -1) * GRID_SIZE,
            dy: (Math.random() < 0.5 ? 1 : -1) * GRID_SIZE
...
function draw() {
    ...
    for (let i = 0; i < snake.length; i++) {
        ...
        if (i === 0) { // 如果是蛇头
            ...
            context.translate(segment.x + SNAKE_SIZE / 2, segment.y + SNAKE_SIZE / 2); // 将绘图原点移至蛇头中心
            
            // 根据移动方向旋转蛇头图像
            if (dx === GRID_SIZE) { // 向右移动
                context.rotate(0);
            } else if (dx === -GRID_SIZE) { // 向左移动
                context.rotate(Math.PI);
            } else if (dy === -GRID_SIZE) { // 向上移动
                context.rotate(-Math.PI / 2);
            } else if (dy === GRID_SIZE) { // 向下移动
                context.rotate(Math.PI / 2);
            }

            context.drawImage(snakeHeadImg, -SNAKE_SIZE / 2, -SNAKE_SIZE / 2, SNAKE_SIZE, SNAKE_SIZE);
            context.restore(); // 恢复之前保存的绘图状态
        } else {
            context.drawImage(snakeBodyImg, segment.x, segment.y, SNAKE_SIZE, SNAKE_SIZE); // 绘制蛇的其他部分
        }
    }

    // 繪製食物
    context.drawImage(foodImg, food.x, food.y, FOOD_SIZE, FOOD_SIZE);
}
